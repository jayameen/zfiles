...
public class ZfileGCS implements Zfile {
    ...
    @Value("${gcp.bucket-id}") private String gcpBucketId;
    @Value("${gcp.upload-url}") private String gcpBaseUrl;
    ...
    public String getFileInBase64(FileRequest request) throws Exception {
        String fullPath = request.getFileUrl().replaceAll(prefixHttpUrl + "/" + gcpBucketId, "");
        BlobId   blobId = BlobId.of(gcpBucketId,fullPath);
    ...
    public Boolean deleteFileByUrl(FileRequest request) throws Exception {
        String fullPath = request.getFileUrl().replaceAll(prefixHttpUrl + "/" + gcpBucketId, "");
        BlobId   blobId = BlobId.of(gcpBucketId, StringUtils.removeStart(fullPath,"/"));
    ...
    private String createFileFromByteArray(FileRequest request) {
        ...
        BlobId blobId  = BlobId.of(gcpBucketId, StringUtils.removeStart(request.getFilePath(), "/") + "/" + request.getFileName());
        String fullUrl = prefixHttpUrl + "/" + gcpBucketId + request.getFilePath() + "/" + request.getFileName();
